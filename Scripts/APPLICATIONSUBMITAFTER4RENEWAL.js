/*
 *   support copy contact address to target cap.
 *   Updated 2012-06-27
 */
...
var capContact = null;
var expirationCode = "YEAR";
var contEduName = "English";
...
function copyKeyInfo(srcCapId, targetCapId)
{
	...
	//Copy Education information from reference contact.
        copyPeopleEducation(srcCapId, targetCapId);
	//Copy Continuing Education information from reference contact.
	copyPeopleContEducation(srcCapId, targetCapId);
	//Copy Examination information from reference contact.
	copyPeopleExamination(srcCapId, targetCapId);
	//Copy Asset
	copyAsset(srcCapId, targetCapId);
}

function getCapContactByContactType(srcCapId, contactType)
{
	if(srcCapId == null || srcCapId == '')
	{
		return;
	}
	...
	var capPeople = null;
	for(loop in capPeoples)
	{
		var model = capPeoples[loop].getCapContactModel();
		if(contactType == model.getContactType())
		{
			capPeople = model;
			break;
		}
...
function isFromRefContact(capContact)
{
	var isFromRef = false;
	if(capContact != null && capContact.getRefContactNumber() != null)
	{
		isFromRef = true;
	}
	return isFromRef;
}
function copyPeopleEducation(srcCapId, targetCapId)
{
	if(srcCapId != null && targetCapId != null)
	{
		capContact = getCapContactByContactType(srcCapId, contactType);
		if(!isFromRefContact(capContact))
		{
			return;
		}
		
		var educations = aa.education.getPeopleEducationList(capContact.getRefContactNumber()).getOutput();
		if(educations.length > 0)
		{
			var srcEduList = aa.util.newArrayList();

			for(loop in educations)
			{
				var eduModel = educations[loop].getEducationModel();
				srcEduList.add(eduModel);
			}
			aa.education.copyPeopleEducationList(srcEduList, targetCapId);
		}
	}
}

// Get the License Expiration
function getLicenseExpiration(capId)
{
	if(capId == null)
	{
		return;
	}
	var expScriptResult = aa.expiration.getLicensesByCapID(capId);
	var expModel = null;
	if(expScriptResult.getSuccess())
	{
		var expScriptModel = expScriptResult.getOutput();
		expModel = expScriptModel.getB1Expiration();
	}
	return expModel;
}

function copyPeopleContEducation(srcCapId, targetCapId)
{
	if(srcCapId != null && targetCapId != null)
	{
		if(!isFromRefContact(capContact))
		...
		var contEducations = aa.continuingEducation.getPeopleContEducationList(capContact.getRefContactNumber()).getOutput();
		if(contEducations.length > 0)
		{
			var srcContEduList = aa.util.newArrayList();
			var expModel = getLicenseExpiration(srcCapId);
			var expDate = null;
			var expCode = "";
			var expInt = 1;
			if(expModel != null)
			{
				expDate = expModel.getExpDate();
				expCode = expModel.getExpCode();
				expInt = expModel.getExpInterval();
			}
			for(loop in contEducations)
			{
			        var contEduModel = contEducations[loop].getContinuingEducationModel();
				// add the filter conditions
				var times = 0;
				var isExpirate = false;
				if(expirationCode == expCode && expDate != null && contEduModel.getDateOfClass() != null)
				{
					times = expInt * 365 * 24 * 3600 * 1000;
					// check continuing education whether is expired.
					if((expDate.getTime() - contEduModel.getDateOfClass().getTime()) > times)
					{
						isExpirate = true;
					}
				}

				if(contEduModel.getContEduName() == contEduName && !isExpirate)
				{
					srcContEduList.add(contEduModel);
				}
			}
			aa.continuingEducation.copyPeopleContEducationList(srcContEduList, targetCapId);
		}
	}
}

function copyPeopleExamination(srcCapId, targetCapId)
{
	if(srcCapId != null && targetCapId != null)
	{
		if(!isFromRefContact(capContact))
		...
		var exams = aa.examination.getPeopleExaminationList(capContact.getRefContactNumber()).getOutput();
		if(exams.length > 0)
		{
			var srcExamList = aa.util.newArrayList();

			for(loop in exams)
			{
				var examModel = exams[loop].getExaminationModel();
				srcExamList.add(examModel);
			}
			aa.examination.copyPeopleExaminationList(srcExamList, targetCapId);
		}
	}
}
...
function copyAsset(srcCapId, targetCapId)
{
	//1. Get Asset with source CAPID.
	var srcAssets = getAsset(srcCapId);
	if (srcAssets == null || srcAssets.length == 0)
	{
		return;
	}
	
	//2. Get Asset with target CAPID.
	var targetAssets = getAsset(targetCapId);
	
	//3. Check to see which Asset is matched in both source and target.
	var newAssetArr = new Array();
	var existingAssetArr = new Array();
	for (loopk in srcAssets)
	{
		var srcAssetModel = srcAssets[loopk];
		if (isAssetEmpty(srcAssetModel))
		{
			continue;
		}
		
		//3.1 Set target CAPID to source Asset.
		srcAssetModel.setCapID(targetCapId);
		var isNewAsset = true;
		
		//3.2 Check to see if source Asset exist.
		if (targetAssets != null && targetAssets.length > 0)
		{
			for (loop2 in targetAssets)
			{
				//3.3 The asset model is existing in targetAssets. So it should be updated. 
				if (isMatchAsset(srcAssetModel, targetAssets[loop2]))
				{
					isNewAsset = false;
					srcAssetModel.setAssetOrder(targetAssets[loop2].getAssetOrder());
					existingAssetArr.push(srcAssetModel);
					break;
				}
			}
		}
		
		//3.4 It is a new Record Asset model. So it should be added.
		if (isNewAsset)
		{
			newAssetArr.push(srcAssetModel);
		}
	}
	
	//4.1 Batch update Record Asset.
	aa.asset.batchUpdateRecordAsset(existingAssetArr);
	//4.2 Batch create Record Asset.
	aa.asset.batchCreateRecordAsset(newAssetArr);
}

function getAsset(capId)
{
	var recordAssetArr = null;
	var s_result = aa.asset.getRecordAssetsByRecordId(capId);
	if(s_result.getSuccess())
	{
		recordAssetArr = s_result.getOutput();
		if (recordAssetArr == null || recordAssetArr.length == 0)
		{
			aa.print("WARNING: no Asset on this CAP:" + capId);
			recordAssetArr = null;
		}
	}
	else
	{
		aa.print("ERROR: Failed to get CAP Asset: " + s_result.getErrorMessage());
		recordAssetArr = null;	
	}
	// Return WorkOrderAssetModel array
	return recordAssetArr;
}

function isMatchAsset(capAssetModel1, capAssetModel2)
{
	if (isAssetEmpty(capAssetModel1) || isAssetEmpty(capAssetModel2))
	{
		return false;
	}
	var assetPK1 = capAssetModel1.getAssetPK();
	var assetPK2 = capAssetModel2.getAssetPK();
	var servProvCode1 = assetPK1.getServiceProviderCode();
	var servProvCode2 = assetPK2.getServiceProviderCode();
	var assetSeqNbr1 = assetPK1.getG1AssetSequenceNumber();
	var assetSeqNbr2 = assetPK2.getG1AssetSequenceNumber();
	
	if (servProvCode1 == servProvCode2 && assetSeqNbr1 == assetSeqNbr2)
	{
		return true;
	}
...
function isAssetEmpty(capAssetModel)
{
	if (capAssetModel == null || capAssetModel.getAssetPK() == null
		|| capAssetModel.getAssetPK().getServiceProviderCode() == null || capAssetModel.getAssetPK().getG1AssetSequenceNumber() == null)
	{
		return true;
	}
