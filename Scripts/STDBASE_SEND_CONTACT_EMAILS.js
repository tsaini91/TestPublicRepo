	...
	if (isConfigurableScript(settingsArray, scriptSuffix)) {
		...
		for (s in settingsArray) {
			...
			if (!isEmptyOrNull(rules.preScript)) {
				logDebug("Running preScript = " + rules.preScript)
...
function sendContactEmails(itemCapId, recordSettings, parameters) {
	...
	var rCustomEmailParamsVariable = handleUndefined(recordSettings.action.customEmailParamsVariable, false);
	...
	if(isEmptyOrNull(rCustomEmailParamsVariable)){
		rCustomEmailParamsVariable = "customEmailParams";
	}
	var eParams = null;
	var envVarCustomEmailParams = aa.env.getValue(rCustomEmailParamsVariable);
	logDebug("envVarCustomEmailParams = " + envVarCustomEmailParams);
